...
int attrcount,closcount=0,fdcount,count_ck=0;
int closure[10];
char attr[10][25],array_ck[100];


void getclosure() {
    int i,j,k,l=0,issubset,found;
    do {
        for(i=0;i<=fdcount;i++) {    //Checking each functional dependency
            issubset=1;
            for(j=0;j<f[i].left_count;j++) {      // This for loop is for checking if left side of i'th FD is a subset of present closure or not
                found=0;
            ...
            if(issubset==1) {                          // If left side of i'th FD is a subset
                for(k=0;k<f[i].right_count;k++) {           // This for loop will add all elements in right side of i'th FD if that element is
...
//out = fopen("output.txt", "w"); 
    ...
    for(i=0;i<10;i++) {
        ...
        do {                                       // This while loop is for getting all left attributes of the FD of outer for loop
            ...
            f[fdcount].left[f[fdcount].left_count]=attcode;                    // Setting the attribute in appropriate position of our fd structure
            f[fdcount].left_count++;                                           // Incrementing the left count for the fd
        } while(temp1[j]==',');
        ...
        do {                                            // This while loop is for getting all right attributes of the FD of outer for loop
            ...
            temp[k]='\0';
            attcode=compare(temp);                                         // Getting the position of temp in attr array
            f[fdcount].right[f[fdcount].right_count]=attcode;                   // Setting the attribute in appropriate position of our fd structure
            f[fdcount].right_count++;                                           // Incrementing theright count for the fd
        } while(temp1[j]==',');
    ...
    cout << endl << endl;
    for (i = 0; i < count; i++) {
                                // this for loop is to print all candidate keys
        ...
        if(closcount != 0) {
            ...
            if(closcount==attrcount && minAttr==p) {
                 ...
                 count_ck++;                   // Print if the current attribute set is a super key with
                cout<<"Candidate key";                          // min nummber of attribute(ie.candidate key)
                for(int q=0; q<p-1; q++){
                    cout <<": ("<< attr[subsetArr[q]] << ", ";
                     myfile <<"CK"<<count_ck<<": ("<< attr[subsetArr[q]] << ", " ;

                cout << attr[subsetArr[p-1]] << ")";
                myfile<< attr[subsetArr[p-1]] << ")";
                cout << endl;
                myfile<<endl;


                //fprintf(out, "CK%d : %c , %c", count_ck,attr[subsetArr[q]],attr[subsetArr[p-1]]);
                }
